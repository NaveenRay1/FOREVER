                  ...
                  <div className='flex gap-2'>
                    {
                      productData.sizes.map((item,index)=>(
                        <button onClick={()=>setSizes(item)} key={index} className={`border py-2 px-4 bg-gray-100 ${item===sizes? 'border-orange-500' : ''}`}>{item}</button>
                      ))
                    }
                  </div>
                  
                </div>
                <button onClick={()=>addToCart(productData._id, sizes)} className='bg-black px-8 py-3 text-white text-sm active:bg-gray-700'>ADD TO CART</button>
                <hr className='mt-8 sm:w-4/5 text-gray-300'></hr>
